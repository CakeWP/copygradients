...
class ControlPointKeyboardMove extends Component {
  ...
  increase(event) {
    ...
    onChange(
      getGradientWithPositionAtIndexIncreased(gradientAST, gradientIndex)
    );
  }
  ...
  decrease(event) {
    ...
    const { gradientIndex, onChange, gradientAST } = this.props;
    onChange(
      getGradientWithPositionAtIndexDecreased(gradientAST, gradientIndex)
    );
  }
  render() {
...
export default function ControlPoints({
  ...
  const onMouseMove = event => {
    ...
    if (!significantMoveHappened) {
      ...
      if (
        Math.abs(initialPosition - relativePosition) >= MINIMUM_SIGNIFICANT_MOVE
      ) {
